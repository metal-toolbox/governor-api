...
echo "Creating NATS account for governor"
nsc add account -n GOVERNOR -K ${OPERATOR_SIGNING_KEY_ID}
nsc edit account GOVERNOR --sk generate --js-mem-storage -1 --js-disk-storage -1 --js-streams -1 --js-consumer -1
nsc describe account GOVERNOR

export ACCOUNTS_SIGNING_KEY_ID=`nsc describe account GOVERNOR -J | jq -r '.nats.signing_keys | first'`

echo "Creating NATS user for governor"
