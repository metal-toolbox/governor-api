...
func (o *NotificationTarget) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(notificationTargetAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(notificationTargetPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert notification_targets, could not build conflict column list")
			}

			conflict = make([]string, len(notificationTargetPrimaryKeyColumns))
			copy(conflict, notificationTargetPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"notification_targets\"", updateOnConflict, ret, update, conflict, insert, opts...)
